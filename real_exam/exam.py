...
while True:
    ...
    dists = sf.dists
    angles = sf.angles
    print("stupid while loop")
    if ids is not None and L1 in ids:
        # Black magic         
        sf.est_pose.theta = math.radians(((360 + math.degrees(math.atan2(l_coords[L1][0] - sf.est_pose.getX() , l_coords[L1][1] - sf.est_pose.getY() ))) % 360) - math.degrees(angles[list(ids).index(L1)]))
        drive_towards_object(angles[list(ids).index(L1)], dists[list(ids).index(L1)])
        break
   # hf.TurnXDegLeft(arlo, 30)
    #time.sleep(1)
...
while len(LANDMARKS) > 0:
            ...
            for id in unique_ids:
                if id not in CHECKED_LANDMARKS:
                    ...
                    if target == 'landmark':
                        ...
                        if id not in VISITED_OBSTACLES and id not in LANDMARKS and id is not None:
    ...
    # Face upwards
    print("___________NOW FACING UPWARDS______________")
    degrwes = 90 - math.degrees(sf.est_pose.getTheta())
    hf.TurnXDegLeft(arlo, degrwes)
    print("degrees: ", degrwes)
    time.sleep(3)
